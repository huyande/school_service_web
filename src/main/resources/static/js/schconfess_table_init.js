$(document).ready(function() {
    ...
    var oTable = $('#confess-table-info').dataTable( {
       ...
       /*"ajax": {
        ...
        "aoColumnDefs": [{
			"targets": -1,
			"data": null,
			"defaultContent": '<a class="btn btn-success btn-del_conf" style="margin-left:60px"  href="javascript:;">禁止显示 </a>'
							+'<a class="btn btn-primary btn-revert_conf" style="margin-left:60px"  href="javascript:;">恢复 </a>'
		}]
    ...
    */
    $('#confess-table-info tbody').on( 'click', '.btn-del_conf', function () {
		var nTr = $(this).parents('tr')[0];
		var aData = oTable.fnGetData( nTr );
		//发送ajax
		$.ajax({
			url:'/admin/confession/confChanageStutas',
			type:'post',
			data:{"confid":aData.id,"funName":"delete"},
			dataType:'json',
    ...
    $('#confess-table-info tbody').on( 'click', '.btn-revert_conf', function () {
    	var nTr = $(this).parents('tr')[0];
    	var aData = oTable.fnGetData( nTr );
    	//发送ajax
		$.ajax({
			url:'/admin/confession/confChanageStutas',
			type:'post',
			data:{"confid":aData.id,"funName":"restore"},
			dataType:'json',
			success:function(data){
				if(data.result){
					//alert(data.result)
